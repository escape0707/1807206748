...
PHONETIC_PATTERN = re.compile(r'<span class="phon">/(.*?)/</span>')


# Register the dict service with a labels to be shown in the dicts list.. Chinese first, English second.
@register(["本地词典-牛津高阶10", "MDX-OALD10"])
class OALD10(MdxService):
    def __init__(self):
        dict_path = DICT_PATH
        if not dict_path:
            # if auto detect DICT_PATH
            ...
            for clazz in service_manager.mdx_services:
                ...
                if title.startswith("牛津高阶英汉双解词典（第10版）V3"):
    ...
    def just_raw_html(self) -> str:
        html = self.get_html()
        # (pathlib.Path.home() / "workspaces/lookup.html").write_text(
        #     html, encoding="utf-8"
        # )
        return html
    ...
    def why_i_cant_use_fld_phonetic(self) -> str:  # TODO
        html = self.get_html()
        m = PHONETIC_PATTERN.search(html)
    ...
    # Recursively jump to the real definition that current word page links to
    ...
    def _field_pronunciation(self, dialect: Dialect) -> str:
        ...
        # get anki's full save path of the file and return it
